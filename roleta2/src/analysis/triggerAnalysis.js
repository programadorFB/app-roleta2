...
import { PHYSICAL_WHEEL, RED_NUMBERS, getRouletteColor } from '../constants/roulette.js';
...
const getColor = getRouletteColor;
...
/**
 * Retorna sinais ativos (disparados nos últimos validFor spins).
...
export function getActiveSignals(spinHistory, triggerMap, validFor = DEFAULT_VALID_FOR) {
  ...
  for (let j = 0; j <= Math.min(validFor, spinHistory.length - 1); j++) {
    ...
    const remaining = validFor - j;
    const status = hit ? 'win' : remaining > 0 ? 'pending' : 'loss';

    signals.push({
      triggerNumber: spinHistory[j].number,
      action: profile.bestPattern.label,
      confidence: profile.bestPattern.confidence,
      lift: profile.bestPattern.lift,
