// components/TriggerStrategiesPanel.jsx — v4 ASSERTIVIDADE POR TIPO
// Visual inspirado: tabela por tipo de gatilho com G1/G2/G3/RED e chips coloridos
...
// Percorre spinHistory e testa cada trigger disparado
// ══════════════════════════════════════════════════════════════
...
function computeAssertivity(spinHistory, triggerMap) {
  ...
  // Per-trigger individual stats
  const perTrigger = new Map(); // triggerNumber -> { g1, g2, g3, red }

  const scanLimit = Math.min(spinHistory.length - 1, spinHistory.length);

  for (let i = LOSS_THRESHOLD; i < scanLimit; i++) {
    ...
    // Track per-trigger
    if (!perTrigger.has(num)) perTrigger.set(num, { g1: 0, g2: 0, g3: 0, red: 0 });
    const pt = perTrigger.get(num);
    if (hitOn === 1) pt.g1++;
    else if (hitOn === 2) pt.g2++;
    else if (hitOn === 3) pt.g3++;
    else pt.red++;
  }

  // Retorna só tipos que tiveram pelo menos 1 disparo
  const result = [];
  for (const [key, data] of Object.entries(types)) {
    ...
    result.push({
      ...
      // Últimos 10 resultados (mais recentes primeiro no array results)
      recentResults: data.results.slice(-10).reverse(),
    });
  }

  result.sort((a, b) => b.pct - a.pct);
  return { types: result, perTrigger };
}
...
const TypeRow = ({ data, isExpanded, onToggle, allTriggers, perTrigger }) => {
  const Icon = data.Icon;
  const pctColor = data.pct >= 60 ? '#34d399' : data.pct >= 40 ? '#c9a052' : '#ef4444';
  const triggersOfType = allTriggers.filter(t => classifyTrigger({ bestPattern: t }) === data.key);
  ...
  return (
    <>
      ...
      {isExpanded && triggersOfType.length > 0 && (
        <div className={styles.typeDetail}>
          <div className={styles.detailGrid}>
            {triggersOfType.slice(0, 12).map(t => {
              const pt = perTrigger.get(t.triggerNumber);
              const ptTotal = pt ? pt.g1 + pt.g2 + pt.g3 + pt.red : 0;
              const ptWins = pt ? pt.g1 + pt.g2 + pt.g3 : 0;
              const ptPct = ptTotal > 0 ? Math.round((ptWins / ptTotal) * 100) : 0;
              const ptColor = ptPct >= 60 ? '#34d399' : ptPct >= 40 ? '#c9a052' : '#ef4444';
              return (
                <div key={t.triggerNumber} className={styles.detailItem}>
                  <NumberChip number={t.triggerNumber} />
                  <span className={styles.detailAction}>{t.label}</span>
                  {ptTotal > 0 && (
                    <span className={styles.detailPct} style={{ color: ptColor }}>
                      {ptPct}%
                      <span className={styles.detailPctSub}>{ptWins}/{ptTotal}</span>
                    </span>
                  )}
                  <span className={styles.detailHeat} style={{ color: getHeatLabel(t.lift).color }}>
          ...
          {triggersOfType.length > 12 && (
            <div className={styles.detailMore}>+{triggersOfType.length - 12} gatilhos</div>
          )}
        </div>
      )}
    </>
...
const TriggerStrategiesPanel = ({ spinHistory, triggerMap: externalTriggerMap }) => {
  ...
  const triggerMap = useMemo(
    () => externalTriggerMap ?? buildTriggerMap(spinHistory, spinHistory.length),
    [externalTriggerMap, spinHistory]
  );
  ...
  const { types: assertivity, perTrigger } = useMemo(
      ...
      <div className={styles.container}>
        <div className={styles.emptyState}>
          ...
          <div className={styles.progressMini}>
            <div className={styles.progressMiniFill} style={{ width: `${(spinHistory.length / 10) * 100}%` }} />
          </div>
          <span className={styles.emptyCount}>{spinHistory.length}/10</span>
        </div>
      </div>
    );
  }
