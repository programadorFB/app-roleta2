...
const StopLossCard = React.memo(({
  ...
  // Cor escalonada: ouro suave → ouro → âmbar → vermelho conforme o risco sobe.
  const getSliderColor = () => {
    if (stopLossPercentage === 0) return '#7F9DB9'; // não definido
    if (stopLossPercentage <= 3) return '#34d399'; // verde — conservador
    if (stopLossPercentage <= 6) return '#c9a052'; // ouro — moderado
    if (stopLossPercentage <= 8) return '#f59e0b'; // âmbar — arriscado
    return '#f87171'; // vermelho — alto risco
  };
  ...
  const getStatusInfo = () => {
    ...
    switch (status) {
      ...
      default:
        return {
          color: '#7F9DB9', // não definido
          icon: <MdHelpOutline />,
          title: 'NÃO DEFINIDO',
          message: 'Configure seu limite de perda para proteção',
      ...
      <div className={styles.content}>
        ...
        <div className={styles.amountSection}>
          <div className={styles.amountItem}>
            <span className={styles.amountLabel}>Valor do Limite</span>
