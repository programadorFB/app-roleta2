...
import { initRedis, redisHealthCheck, closeRedis, cacheSet, KEY, TTL, getPubSubClients } from './redisService.js';
import {
  processHublaWebhook, verifyHublaWebhook,
...
import { processSource } from './motorScoreEngine.js';
...
function generateETag(data) {
  ...
  if (Array.isArray(data) && data.length > 0) {
    ...
    hash.update(`${first?.signalId || first?.signalid || ''}:${last?.signalId || last?.signalid || ''}:${data.length}`);
  } else {
    hash.update('empty');
  }
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://tool-api.smartanalise.com.br", "wss://tool-api.smartanalise.com.br", "https://ferramenta-api.smartanalise.com.br", "wss://ferramenta-api.smartanalise.com.br"],
      fontSrc:    ["'self'", "data:"],
...
const FAIL_OPEN = process.env.SUBSCRIPTION_FAIL_OPEN !== 'false'; // default: true (fail-open)
...
async function checkSubscriptionWithFallback(email) {
  ...
  try {
    ...
    console.error(`⚠️ [checkSub] Erro no fresh DB check — ${FAIL_OPEN ? 'fail-open' : 'fail-closed'}:`, dbErr.message);
    Sentry.captureException(dbErr, { tags: { context: 'subscription-fresh-check' }, extra: { email } });
    return { canPlay: FAIL_OPEN, subscription: null };
  }
}
